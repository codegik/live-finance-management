...
**Interfaces:**
- Consumes: nothing.
- Produces: `createDb(url: string): Db` and `getDb(): Db` from `lib/db/client.ts`, where `Db = NodePgDatabase<typeof schema>`; `env` object from `lib/env.ts`; `testDb(): Db` and `resetDb(): Promise<void>` from `tests/helpers/db.ts`; Vitest injected key `databaseUrl`.
