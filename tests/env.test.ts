...
it('accepts a fully populated environment', () => {
  expect(() => loadEnv(valid as unknown as NodeJS.ProcessEnv)).not.toThrow()
})
...
it('rejects the published .env.example placeholder even though it is long enough', () => {
  ...
  expect(() =>
    loadEnv({ ...valid, AUTH_SECRET: placeholder } as unknown as NodeJS.ProcessEnv),
...
it('rejects CHANGE_ME in any of the secrets', () => {
  for (const key of ['AUTH_SECRET', 'PLUGGY_WEBHOOK_TOKEN', 'CRON_SECRET'] as const) {
    expect(() =>
      loadEnv({ ...valid, [key]: 'CHANGE_ME' } as unknown as NodeJS.ProcessEnv),
    ).toThrow(/placeholder/i)
  }
})
...
it('rejects a CHANGE_ME variant with a suffix', () => {
  expect(() =>
    loadEnv({ ...valid, CRON_SECRET: 'CHANGE_ME_CRON_SECRET_PLEASE' } as unknown as NodeJS.ProcessEnv),
  ).toThrow(/placeholder/i)
})

it('rejects the Pluggy credential placeholders', () => {
  expect(() =>
    loadEnv({ ...valid, PLUGGY_CLIENT_ID: 'replace-with-your-pluggy-client-id' } as unknown as NodeJS.ProcessEnv),
  ).toThrow(/placeholder/i)
})

it('requires the alert channel to be configured', () => {
  const { RESEND_API_KEY, ...withoutKey } = valid
  expect(() => loadEnv(withoutKey as unknown as NodeJS.ProcessEnv)).toThrow()
